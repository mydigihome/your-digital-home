...
export default function SettingsPage() {
  ...
  const [reviewMonth] = useState(new Date().getMonth() + 1);
  ...
  // KEY FIX: saveProfile now updates profiles table + auth metadata so name propagates everywhere immediately
  const saveProfile = async () => {
    if (!user) return;
    setSaving(true);
    try {
      // 1. Update profiles table with full_name, handle, location
      ...
      // 2. Sync full_name into Supabase Auth user_metadata
      // This is what useAuth reads from profile.full_name via fetchProfile
      // and also what the dashboard greeting uses
      if (profileData.full_name.trim()) {
        await supabase.auth.updateUser({
          data: { full_name: profileData.full_name.trim() },
        });
      }

      // 3. Update email if it changed
      if (profileData.email.trim() && profileData.email.trim() !== user.email) {
        await supabase.auth.updateUser({ email: profileData.email.trim() });
        toast.success("Profile saved! Check your new email for a confirmation link.");
      } else {
        toast.success(`Profile saved! Dashboard will now greet you as ${profileData.full_name.trim() || "you"}.`);
      }
    } catch (err: any) {
  ...
  const saveAppearance = () => {
    ...
    upsertPrefs.mutate({ theme_color: theme.primary, secondary_color: theme.secondary, sidebar_theme: isDarkMode ? "dark" : "light", religion: selectedReligion || null, show_scripture_card: showScripture, welcome_video_url: welcomeVideoUrl || null } as any);
  ...
  const toggleDarkMode = () => {
    ...
    if (next) { document.documentElement.classList.add("dark"); document.body.classList.add("dark"); }
  ...
  const submitFeedback = async () => {
    if (!user) return;
    if (!feedbackMessage.trim()) { toast.error("Write something first"); return; }
    await (supabase as any).from("feedback").insert({ user_id: user.id, email: user.email, message: feedbackMessage.trim(), rating: null, status: "pending" });
    setFeedbackMessage(""); setFeedbackType("General Feedback");
    ...
    <AppShell>
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "32px 16px 100px" }}>
        ...
        <div style={{ display: "flex", gap: 4, marginBottom: 24, borderBottom: `1px solid ${border}`, paddingBottom: 0, overflowX: "auto" }}>
          {TABS.map(tab => (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)} style={{ padding: "8px 16px", fontSize: 13, fontWeight: activeTab === tab.key ? 600 : 500, whiteSpace: "nowrap" as const, color: activeTab === tab.key ? (localStorage.getItem("dh_accent_color") || "#10B981") : text2, borderBottom: activeTab === tab.key ? `2px solid ${localStorage.getItem("dh_accent_color") || "#10B981"}` : "2px solid transparent", background: "transparent", border: "none", cursor: "pointer", fontFamily: "Inter, sans-serif", marginBottom: -1, transition: "all 150ms" }}>
              {tab.label}
            </button>
          ))}
        </div>
            ...
            <div style={sectionStyle}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 20 }}>
                <div onClick={() => avatarInputRef.current?.click()} style={{ width: 72, height: 72, borderRadius: "50%", background: isDark ? "#252528" : "#F3F4F6", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", overflow: "hidden", border: `2px solid ${inputBorder}`, flexShrink: 0 }}>
                  {avatarUrl ? <img src={avatarUrl} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <span style={{ fontSize: 24, fontWeight: 700, color: text2 }}>{profileData.full_name?.charAt(0)?.toUpperCase() || user?.email?.charAt(0).toUpperCase()}</span>}
                </div>
                <div>
              ...
              <p style={{ fontSize: 12, color: text2, fontFamily: "Inter, sans-serif", marginTop: 10 }}>\uD83D\uDCA1 Your Full Name will appear on your dashboard greeting and throughout the app.</p>
              <button onClick={saveProfile} disabled={saving} style={{ marginTop: 12, padding: "10px 24px", background: "#10B981", color: "white", border: "none", borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "Inter, sans-serif" }}>{saving ? "Saving..." : "Save Profile"}</button>
            </div>
              ...
              {savedReviews.length === 0 ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {savedReviews.map((review: any) => (
                    <div key={review.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 16px", background: isDark ? "#252528" : "white", border: `1px solid ${isDark ? "rgba(255,255,255,0.06)" : "#F3F4F6"}`, borderRadius: 12 }}>
                      <div>
                        ...
                        {review.ai_summary && <p style={{ fontSize: 12, color: text2, fontFamily: "Inter, sans-serif", fontStyle: "italic", margin: "4px 0 0" }}>"{ (review.went_well || review.ai_summary || "").substring(0, 80)}..."</p>}
                      </div>
                      <div style={{ display: "flex", gap: 6 }}>
                        ...
                        <button onClick={async () => { if (!window.confirm("Delete?")) return; await (supabase as any).from("monthly_reviews").delete().eq("id", review.id).eq("user_id", user!.id); setSavedReviews(prev => prev.filter((r: any) => r.id !== review.id)); toast.success("Review deleted"); }} style={{ width: 32, height: 32, background: "transparent", border: `1px solid ${isDark ? "rgba(255,255,255,0.06)" : "#F3F4F6"}`, borderRadius: 8, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: text2 }}>
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </div>
        ...
        {activeTab === "connections" && (
          <div style={sectionStyle}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 0", borderBottom: `1px solid ${border}` }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ width: 38, height: 38, borderRadius: 10, background: "rgba(66,133,244,0.1)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                ...
                <div>
                  ...
                  <span style={{ fontSize: 12, color: text2, fontFamily: "Inter, sans-serif" }}>{gcalConnection.data?.calendar_email || "Sync events to Today's Agenda"}</span>
                </div>
              </div>
              {gcalConnection.data ? (
                <button onClick={() => disconnectGcal.mutate()} disabled={disconnectGcal.isPending} style={{ padding: "7px 16px", background: inputBg, border: `1.5px solid ${inputBorder}`, borderRadius: 8, fontSize: 12, fontWeight: 600, color: "#DC2626", cursor: "pointer", fontFamily: "Inter, sans-serif" }}>{disconnectGcal.isPending ? "..." : "Disconnect"}</button>
              ) : (
                <button onClick={connectGcal} disabled={gcalConnecting} style={{ padding: "7px 16px", background: inputBg, border: `1.5px solid ${inputBorder}`, borderRadius: 8, fontSize: 12, fontWeight: 600, color: text1, cursor: "pointer", fontFamily: "Inter, sans-serif" }}>{gcalConnecting ? "Connecting..." : "Connect"}</button>
              )}
            </div>
          </div>
        )}
        ...
        {activeTab === "billing" && (
          <div style={sectionStyle}>
            ...
            <div style={{ display: "flex", gap: 0, marginBottom: 28, background: isDark ? "#252528" : "#F3F4F6", borderRadius: 12, padding: 5, width: "fit-content" }}>
              <button onClick={() => setBillingCycle("monthly")} style={{ padding: "9px 24px", borderRadius: 8, border: "none", background: billingCycle === "monthly" ? (isDark ? "#333" : "white") : "transparent", fontSize: 14, fontWeight: billingCycle === "monthly" ? 600 : 400, color: billingCycle === "monthly" ? text1 : text2, cursor: "pointer", fontFamily: "Inter, sans-serif" }}>Monthly</button>
              <button onClick={() => setBillingCycle("annual")} style={{ padding: "9px 24px", borderRadius: 8, border: "none", background: billingCycle === "annual" ? (isDark ? "#333" : "white") : "transparent", fontSize: 14, fontWeight: billingCycle === "annual" ? 600 : 400, color: billingCycle === "annual" ? text1 : text2, cursor: "pointer", fontFamily: "Inter, sans-serif", display: "flex", alignItems: "center", gap: 6 }}>
                Annual <span style={{ padding: "2px 8px", background: billingCycle === "annual" ? "#F0FDF4" : "#E5E7EB", color: billingCycle === "annual" ? "#065F46" : text2, borderRadius: 999, fontSize: 10, fontWeight: 700 }}>Save 28%</span>
              </button>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, maxWidth: 600, margin: "0 auto 24px" }}>
              {BILLING_PLANS.map(plan => (
                <div key={plan.tier} style={{ border: `2px solid ${currentPlan === plan.tier ? plan.color : (isDark ? "rgba(255,255,255,0.1)" : "#E5E7EB")}`, borderRadius: 18, padding: 20, position: "relative", background: currentPlan === plan.tier ? (isDark ? "rgba(255,255,255,0.03)" : plan.bg) : (isDark ? "#1C1C1E" : "white"), display: "flex", flexDirection: "column" as const }}>
                  {plan.badge && <div style={{ position: "absolute", top: -11, left: "50%", transform: "translateX(-50%)", background: plan.color, color: "white", fontSize: 10, fontWeight: 700, padding: "3px 12px", borderRadius: 999, whiteSpace: "nowrap" }}>{plan.badge}</div>}
                  {(plan as any).badgeCustom && !plan.badge && <div style={{ position: "absolute", top: -11, left: "50%", transform: "translateX(-50%)", background: "#FEF2F2", color: "#DC2626", fontSize: 10, fontWeight: 700, padding: "3px 12px", borderRadius: 999, whiteSpace: "nowrap", border: "1px solid #FECACA" }}>{(plan as any).badgeCustom}</div>}
                  ...
                  <p style={{ fontSize: 32, fontWeight: 800, color: text1, fontFamily: "Inter, sans-serif", letterSpacing: "-1px", marginBottom: 4 }}>${billingCycle === "annual" ? plan.annualPrice : plan.monthlyPrice}<span style={{ fontSize: 13, color: text2, fontWeight: 400 }}>/{billingCycle === "annual" ? "yr" : "mo"}</span></p>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 18 }}>
                    {plan.features.map((f, i) => <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 6 }}><Check size={12} color={plan.color} style={{ flexShrink: 0, marginTop: 2 }} /><span style={{ fontSize: 11, color: isDark ? "rgba(255,255,255,0.6)" : "#374151", fontFamily: "Inter, sans-serif" }}>{f}</span></div>)}
                  </div>
                  {currentPlan === plan.tier ? (
                    <div style={{ padding: 10, background: plan.color + "15", borderRadius: 10, textAlign: "center", fontSize: 13, fontWeight: 700, color: plan.color, marginTop: "auto" }}>Current Plan</div>
                  ) : (
                    <button onClick={() => { window.location.href = billingCycle === "annual" ? plan.stripeAnnual : plan.stripeMonthly; }} style={{ width: "100%", padding: 10, background: plan.color, color: "white", border: "none", borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: "pointer", marginTop: "auto" }}>Get {plan.name}</button>
                  )}
                </div>
              ))}
            </div>
            ...
            {archivedProjects.length === 0 ? (
              ...
              <div key={item.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderBottom: `1px solid ${border}` }}>
                <div>
                  ...
                  <span style={{ fontSize: 11, color: text2, fontFamily: "Inter, sans-serif", display: "block" }}>Archived {new Date(item.updated_at).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span>
                </div>
            ...
            <div style={sectionStyle}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>
                {["Bug Report", "Feature Request", "General Feedback", "Billing Issue"].map(type => (
                  <button key={type} onClick={() => setFeedbackType(type)} style={{ padding: "6px 14px", borderRadius: 999, border: `1.5px solid ${feedbackType === type ? "#10B981" : inputBorder}`, background: feedbackType === type ? (isDark ? "rgba(16,185,129,0.15)" : "#F0FDF4") : inputBg, color: feedbackType === type ? (isDark ? "#10B981" : "#065F46") : text2, fontSize: 12, fontWeight: feedbackType === type ? 600 : 400, cursor: "pointer", fontFamily: "Inter, sans-serif" }}>{type}</button>
                ))}
              </div>
              <textarea value={feedbackMessage} onChange={e => setFeedbackMessage(e.target.value)} placeholder="Tell us what's on your mind..." rows={4} style={{ ...inputStyle, resize: "vertical" as const, marginBottom: 12 }} />
              <button onClick={submitFeedback} style={{ padding: "10px 24px", background: "#10B981", color: "white", border: "none", borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "Inter, sans-serif" }}>Send Feedback</button>
            </div>
          </>
        )}
      </div>
      ...
      {substackModalOpen && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div style={{ background: isDark ? "#1C1C1E" : "white", borderRadius: 16, padding: 24, maxWidth: 400, width: "100%" }}>
            <h3 style={{ fontSize: 16, fontWeight: 700, color: text1, fontFamily: "Inter, sans-serif", marginBottom: 16 }}>Substack Email</h3>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
              <button onClick={() => setSubstackModalOpen(false)} style={{ flex: 1, padding: 10, border: `1.5px solid ${inputBorder}`, borderRadius: 10, background: inputBg, fontSize: 13, fontWeight: 500, color: text1, cursor: "pointer" }}>Cancel</button>
              <button onClick={saveSubstack} style={{ flex: 1, padding: 10, background: "#FF6719", color: "white", border: "none", borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Save</button>
            </div>
          </div>
        </div>
      )}
      ...
      {brokerModalOpen && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div style={{ background: isDark ? "#1C1C1E" : "white", borderRadius: 16, padding: 24, maxWidth: 400, width: "100%" }}>
            <h3 style={{ fontSize: 16, fontWeight: 700, color: text1, fontFamily: "Inter, sans-serif", marginBottom: 16 }}>Preferred Broker</h3>
            <select value={preferredBroker} onChange={e => { setPreferredBroker(e.target.value); const b = BROKERS.find(br => br.name === e.target.value); if (b) setBrokerUrl(b.url); }} style={{ ...inputStyle, cursor: "pointer", marginBottom: 10 }}>
              <option value="">Select a broker...</option>
              {BROKERS.map(b => <option key={b.name} value={b.name}>{b.name}</option>)}
              <option value="Custom">Custom URL</option>
            </select>
            {preferredBroker === "Custom" && <input value={brokerUrl} onChange={e => setBrokerUrl(e.target.value)} placeholder="https://yourbroker.com" style={{ ...inputStyle, marginBottom: 10 }} />}
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setBrokerModalOpen(false)} style={{ flex: 1, padding: 10, border: `1.5px solid ${inputBorder}`, borderRadius: 10, background: inputBg, fontSize: 13, cursor: "pointer" }}>Cancel</button>
              <button onClick={saveBroker} style={{ flex: 1, padding: 10, background: "#10B981", color: "white", border: "none", borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Save</button>
            </div>
          </div>
        </div>
      )}
      ...
      {plaidModalOpen && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div style={{ background: isDark ? "#1C1C1E" : "white", borderRadius: 16, padding: 24, maxWidth: 400, width: "100%" }}>
            <h3 style={{ fontSize: 16, fontWeight: 700, color: text1, fontFamily: "Inter, sans-serif", marginBottom: 12 }}>Connect Your Bank</h3>
            <p style={{ fontSize: 13, color: text2, fontFamily: "Inter, sans-serif", marginBottom: 20 }}>Bank connection requires Plaid setup. Contact support to enable it for your account.</p>
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => window.open("https://plaid.com", "_blank")} style={{ flex: 1, padding: 10, background: "#3B82F6", color: "white", border: "none", borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Learn More</button>
              <button onClick={() => setPlaidModalOpen(false)} style={{ flex: 1, padding: 10, border: `1.5px solid ${inputBorder}`, borderRadius: 10, background: inputBg, fontSize: 13, cursor: "pointer", color: text1 }}>Got it</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div style={{ background: isDark ? "#1C1C1E" : "white", borderRadius: 16, padding: 24, maxWidth: 440, width: "100%" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              ...
              <button onClick={() => setAppleCalModalOpen(false)} style={{ background: "none", border: "none", cursor: "pointer" }}><X size={18} color={text2} /></button>
            </div>
            <p style={{ fontSize: 13, color: text2, marginBottom: 16 }}>Enable CalDAV sync in Apple Calendar settings and paste your CalDAV URL below.</p>
            <input value={caldavUrl} onChange={e => setCaldavUrl(e.target.value)} placeholder="https://caldav.icloud.com/..." style={{ ...inputStyle, marginBottom: 14 }} />
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => { if (!caldavUrl.trim()) { toast.error("Enter a CalDAV URL"); return; } localStorage.setItem("dh_caldav_url", caldavUrl.trim()); setAppleCalModalOpen(false); toast.success("Apple Calendar connected"); }} style={{ flex: 1, padding: "10px 16px", background: "#FF3B30", color: "white", border: "none", borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Save</button>
              <button onClick={() => setAppleCalModalOpen(false)} style={{ flex: 1, padding: "10px 16px", border: `1.5px solid ${inputBorder}`, borderRadius: 10, background: inputBg, fontSize: 13, cursor: "pointer", color: text1 }}>Cancel</button>
            </div>
          </div>
        </div>
      )}
