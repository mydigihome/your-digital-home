...
import { Plus, Upload, Trash2, Download, Sparkles, FileSpreadsheet, Loader2, CheckSquare, X } from "lucide-react";
...
export default function TaxReceiptTracker({ inModal }: Props) {
  const { user } = useAuth();
  ...
  const deleteReceipt = useMutation({
    mutationFn: async (id: string) => { await (supabase as any).from("tax_receipts").delete().eq("id", id); },
  ...
  const scanWithAI = async (file: File) => {
    ...
    try {
      ...
      const preview: Partial<Receipt> = { ...parsed, tax_year: selectedYear, receipt_image_url: imageUrl, ai_extracted: true, source: "scan" };
      setScanStage("done");
      setScanPreview(preview);
      setForm(preview);
      setTimeout(() => { setShowAdd(true); setScanning(false); }, 800);
  ...
  const exportCSV = () => {
    const list = filterCat === "all" ? receipts : receipts.filter(r => r.category === filterCat);
    const headers = ["Date", "Vendor", "Description", "Category", "Subcategory", "Amount", "Deductible", "Notes"];
    const rows = list.map(r => [r.receipt_date || "", r.vendor || "", r.description || "", r.category || "", r.subcategory || "", r.amount !== null ? r.amount.toFixed(2) : "", r.deductible ? "Yes" : "No", r.notes || ""]);
    ...
    toast.success(`Exported ${list.length} receipts`);
  };
  ...
  const catBreakdown = TAX_CATEGORIES.map(c => ({ ...c, total: receipts.filter(r => r.category === c.id).reduce((s, r) => s + (r.amount || 0), 0), count: receipts.filter(r => r.category === c.id).length })).filter(c => c.count > 0);
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex gap-1">
          {years.map(y => (
            <button key={y} onClick={() => setSelectedYear(y)}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition ${selectedYear === y ? "bg-amber-500 text-white" : "bg-muted text-muted-foreground hover:text-foreground"}`}>{y}</button>
          ))}
        </div>
        <div className="ml-auto flex items-center gap-2 flex-wrap">
          <select value={filterCat} onChange={e => setFilterCat(e.target.value)} className="text-xs px-3 py-1.5 rounded-lg border border-border bg-background text-foreground">
          ...
          {receipts.length > 0 && (
            <button onClick={exportCSV} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold border border-border text-foreground hover:bg-muted transition">
      ...
      {/* Hidden inputs */}
      <input ref={cameraRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={e => { const f = e.target.files?.[0]; if (f) scanWithAI(f); }} />
      <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={e => { const f = e.target.files?.[0]; if (f) scanWithAI(f); }} />

      {/* Scan animation */}
      {scanning && scanImagePreview && (
        <div className="relative rounded-xl overflow-hidden border border-border">
          <img src={scanImagePreview} alt="Receipt" className="w-full max-h-56 object-contain bg-black" />
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/40">
            ...
            {scanStage === "analyzing" && (
              <>
                <div className="absolute inset-x-0 h-0.5 bg-green-400 shadow-[0_0_12px_4px_rgba(74,222,128,0.7)]" style={{ animation: "scanLine 1.2s ease-in-out infinite" }} />
            ...
            {scanStage === "done" && (
              <div className="flex flex-col items-center gap-2">
                <div className="w-14 h-14 rounded-full bg-green-500 flex items-center justify-center">
                  <svg width="28" height="28" viewBox="0 0 28 28" fill="none"><path d="M5 14L11 20L23 8" stroke="white" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" /></svg>
                </div>
        ...
        <div className="flex gap-2 flex-wrap">
          {catBreakdown.map(c => (
            <button key={c.id} onClick={() => setFilterCat(filterCat === c.id ? "all" : c.id)}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border transition ${filterCat === c.id ? "text-white border-transparent" : "border-border text-muted-foreground hover:text-foreground"}`}
          ...
          {filtered.map(r => {
            ...
            return (
              <div key={r.id} className="group flex items-center gap-3 p-3 rounded-xl border border-border hover:bg-muted/20 transition">
                ...
                {r.receipt_image_url && <img src={r.receipt_image_url} alt="" className="w-8 h-8 rounded-md object-cover border border-border flex-shrink-0" />}
          ...
          <div className="w-full max-w-md bg-card border border-border rounded-2xl shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-border">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base font-semibold text-foreground">{scanPreview ? "Review Scanned Receipt" : "Add Receipt"}</h3>
              </div>
            ...
            <div className="p-5 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><label className="text-xs font-medium text-muted-foreground block mb-1">Amount ($)</label>
                  <input value={form.amount ?? ""} onChange={e => setForm(f => ({ ...f, amount: parseFloat(e.target.value) || undefined }))} type="number" step="0.01" className="w-full text-sm px-3 py-2 rounded-lg border border-border bg-background text-foreground" /></div>
