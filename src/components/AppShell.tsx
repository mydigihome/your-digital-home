...
function SidebarNav({ onNavigate, collapsed = false }: { onNavigate?: () => void; collapsed?: boolean }) {
  ...
  useEffect(() => {
    ...
    else if (saved === "light") { document.documentElement.classList.remove("dark"); document.body.classList.remove("dark"); setDarkMode(false); }
    else { const prefersDark = window.matchMedia("(prefers-color-scheme: dark)").matches; if (prefersDark) { document.documentElement.classList.add("dark"); document.body.classList.add("dark"); setDarkMode(true); } else { setDarkMode(document.documentElement.classList.contains("dark")); } }
  }, []);
  ...
  const avatarUrl = (prefs as any)?.profile_photo;
  ...
  const navItemClass = (isActive: boolean) => cn(
    ...
    isActive ? "border-l-[3px] border-[#10B981] font-medium" : "border-l-[3px] border-transparent hover:border-[#10B981]",
  );
  ...
  return (
    <div className="flex flex-1 flex-col h-full">
        ...
        <div className="flex items-center gap-2.5">
          ...
          {!collapsed && (
            <>
              ...
              <button onClick={() => setCollapsed(true)} className="shrink-0 w-7 h-7 rounded-lg flex items-center justify-center hover:bg-white/10 transition-colors" style={{ color: 'rgba(255,255,255,0.5)' }}><PanelLeftClose className="w-4 h-4" /></button>
            </>
          )}
      ...
      <div className="shrink-0 px-3 pb-4 relative">
        <button onClick={() => setProfileMenuOpen(!profileMenuOpen)} className={cn("w-full flex items-center rounded-2xl p-3 text-left transition-all duration-200 hover:bg-white/5", collapsed ? "justify-center" : "gap-3")} style={{ border: collapsed ? 'none' : '1px solid rgba(255,255,255,0.08)' }}>
          <div className="relative shrink-0">
            <div className="h-9 w-9 overflow-hidden rounded-full bg-white/10">
              {avatarUrl ? <img src={avatarUrl} alt="Avatar" className="h-full w-full object-cover" /> : <div className="flex h-full w-full items-center justify-center text-xs font-semibold bg-[#10B981] text-white">{displayName.charAt(0).toUpperCase()}</div>}
            </div>
            <div className="absolute bottom-0 right-0 rounded-full border-2 border-[#1C1C1E]" style={{ width: 10, height: 10, backgroundColor: '#22C55E' }} />
          </div>
          {!collapsed && (
            <>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5 min-w-0">
                  <span className="truncate text-[14px] font-normal text-white">{displayName}</span>
                  {isFoundingMember && <Star className="w-3.5 h-3.5 fill-[#F59E0B] text-[#F59E0B] flex-shrink-0" />}
                </div>
                <div className="truncate text-[11px]" style={{ color: 'rgba(255,255,255,0.6)' }}>{user?.email || ''}</div>
              </div>
              <div role="button" onClick={(e) => { e.stopPropagation(); toggleDarkMode(); }} className="shrink-0 inline-flex items-center justify-center rounded-full transition-colors hover:bg-white/10 w-8 h-8 bg-white/5">
                {darkMode ? <Sun className="w-3.5 h-3.5 text-amber-400" /> : <Moon className="w-3.5 h-3.5" style={{ color: 'rgba(255,255,255,0.6)' }} />}
              </div>
            </>
          )}
        </button>
        ...
        <AnimatePresence>
          {profileMenuOpen && (
            <>
              ...
              <motion.div initial={{ opacity: 0, y: 8, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8, scale: 0.95 }} transition={{ duration: 0.15 }} className={cn("absolute bottom-full mb-2 z-50 rounded-xl border border-border bg-card p-1.5 shadow-lg", collapsed ? "left-0 right-0" : "left-3 right-3")} style={{ minWidth: collapsed ? 200 : undefined }}>
                <button onClick={(e) => { e.stopPropagation(); go("/settings"); setProfileMenuOpen(false); }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-secondary transition text-left text-[13px] text-foreground">
                  <div className="w-8 h-8 rounded-full bg-secondary border border-border flex items-center justify-center"><Settings className="w-[18px] h-[18px] text-muted-foreground" strokeWidth={1.5} /></div>Settings
                </button>
                <div className="my-1 border-t border-border" />
                <button onClick={async (e) => { e.stopPropagation(); await signOut(); navigate("/login"); onNavigate?.(); setProfileMenuOpen(false); }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-red-50 transition text-left text-[13px]" style={{ color: '#DC2626' }}>
                  <div className="w-8 h-8 rounded-full bg-red-50 border border-red-200 flex items-center justify-center"><LogOut className="w-[18px] h-[18px] text-red-500" strokeWidth={1.5} /></div>Log out
                </button>
              </motion.div>
            </>
          )}
        ...
        {tabs.map((tab) => {
            ...
            return (
              <div key="more" className="relative flex-1" ref={moreRef}>
                <button onClick={() => setMoreOpen(!moreOpen)} className={cn("mobile-tab-item flex w-full flex-col items-center justify-center gap-0.5 py-2", active ? "text-primary" : "text-muted-foreground")}>
                  <Icon className="h-5 w-5" strokeWidth={active ? 2 : 1.5} /><span className="text-[10px] font-medium">{tab.label}</span>
                </button>
                <AnimatePresence>
                  {moreOpen && (
                    <motion.div initial={{ opacity: 0, y: 8, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8, scale: 0.95 }} transition={{ duration: 0.15 }} className="absolute bottom-full right-0 mb-2 mr-1 w-48 rounded-2xl border border-border bg-card p-1.5 shadow-xl">
                      {moreItems.map((item) => {
                        const ItemIcon = item.icon;
                        const itemActive = location.pathname.startsWith(item.path);
                        return <button key={item.path} onClick={() => { setMoreOpen(false); navigate(item.path); }} className={cn("flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium transition-colors", itemActive ? "bg-accent text-accent-foreground" : "text-foreground hover:bg-secondary")}><ItemIcon className="h-5 w-5" />{item.label}</button>;
                      })}
                    </motion.div>
                  )}
                </AnimatePresence>
          ...
          return (
            <button key={tab.path} onClick={() => navigate(tab.path)} className={cn("mobile-tab-item flex flex-1 flex-col items-center justify-center gap-0.5 py-2", active ? "text-primary" : "text-muted-foreground")}>
              <Icon className="h-5 w-5" strokeWidth={active ? 2 : 1.5} /><span className="text-[10px] font-medium">{tab.label}</span>
              {active && <motion.div layoutId="tab-indicator" className="absolute top-0 h-[2px] w-8 rounded-full bg-primary" transition={{ type: "spring", stiffness: 400, damping: 30 }} />}
            </button>
          );
        })}
      </nav>
    ...
    <SidebarContext.Provider value={{ collapsed, setCollapsed }}>
      <div className="min-h-screen bg-background">
        ...
        <div className="mobile-header sticky top-0 z-40 flex h-12 items-center gap-x-3 border-b border-border bg-card/95 px-4 backdrop-blur-xl lg:hidden">
          <button onClick={() => setMobileOpen(!mobileOpen)} className="rounded-md p-1.5 text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground">
          ...
          {user && (
            <button className="notif-bell" onClick={() => setNotifOpen(!notifOpen)} style={{ position: "relative", width: "34px", height: "34px", borderRadius: "50%", border: `1px solid ${isDark ? "rgba(255,255,255,0.1)" : "#E5E7EB"}`, background: isDark ? "#252528" : "white", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
              ...
              {unreadCount > 0 && <div style={{ position: "absolute", top: "-2px", right: "-2px", width: "16px", height: "16px", borderRadius: "999px", background: "#EF4444", border: `2px solid ${isDark ? "#252528" : "white"}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "9px", fontWeight: "700", color: "white" }}>{unreadCount > 99 ? "99+" : unreadCount}</div>}
            </button>
          )}
        </div>
        ...
        <MobileTabBar />
        <style>{`@media (max-width: 1023px) { main { padding-left: 0 !important; padding-bottom: 72px !important; } }`}</style>
      </div>
