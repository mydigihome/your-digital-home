...
function SidebarNav({ onNavigate, collapsed = false }: { onNavigate?: () => void; collapsed?: boolean }) {
  ...
  // FIX: Admin check by email, not user_roles table
  ...
  useEffect(() => {
    ...
    if (saved === "dark") {
      ...
      // Default to LIGHT mode — never follow system dark preference
      document.documentElement.classList.remove("dark");
      document.body.classList.remove("dark");
      setDarkMode(false);
    }
  ...
  // FIX: Use profile.full_name first, then user metadata, then email prefix — never raw email
  ...
  const navItemHoverHandlers = (isActive: boolean) => ({
    onMouseEnter: (e: React.MouseEvent<HTMLButtonElement>) => {
      if (!isActive) { e.currentTarget.style.backgroundColor = 'rgba(16,185,129,0.08)'; e.currentTarget.style.color = '#FFFFFF'; }
    },
    onMouseLeave: (e: React.MouseEvent<HTMLButtonElement>) => {
      if (!isActive) { e.currentTarget.style.backgroundColor = ''; e.currentTarget.style.color = 'rgba(255,255,255,0.85)'; }
    },
  });
    ...
    <div className="flex flex-1 flex-col h-full">
        ...
        <div className="flex items-center gap-2.5">
          ...
          {!collapsed && (
            <>
              ...
              <button onClick={() => setCollapsed(true)} className="shrink-0 w-7 h-7 rounded-lg flex items-center justify-center hover:bg-white/10 transition-colors" style={{ color: 'rgba(255,255,255,0.5)' }}>
                <PanelLeftClose className="w-4 h-4" />
              </button>
            </>
          )}
          {collapsed && (
            <button onClick={() => setCollapsed(false)} className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-white/10 transition-colors" style={{ color: 'rgba(255,255,255,0.5)' }}>
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-2">
        <ul className="space-y-1">
          ...
          {/* FIX: Admin tab — email-based check, always visible for myslimher@gmail.com */}
          {isAdmin && (
            <li>
              ...
              <div className="relative">
                <div className="h-9 w-9 overflow-hidden rounded-full bg-white/10">
                  {avatarUrl ? <img src={avatarUrl} alt="Avatar" className="h-full w-full object-cover" /> : (
                    <div className="flex h-full w-full items-center justify-center text-xs font-semibold bg-[#10B981] text-white">{displayName.charAt(0).toUpperCase()}</div>
                  )}
                </div>
            ...
            <>
              ...
              <motion.div initial={{ opacity: 0, y: 8, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8, scale: 0.95 }} transition={{ duration: 0.15 }}
                ...
                <button onClick={(e) => { e.stopPropagation(); go("/settings"); setProfileMenuOpen(false); }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-secondary transition text-left text-[13px] text-foreground">
                  <div className="w-8 h-8 rounded-full bg-secondary border border-border flex items-center justify-center"><Settings className="w-[18px] h-[18px] text-muted-foreground" strokeWidth={1.5} /></div>
                  Settings
                </button>
                <button onClick={(e) => { e.stopPropagation(); setProfileMenuOpen(false); navigate("/settings"); }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-secondary transition text-left text-[13px] text-foreground">
                  <div className="w-8 h-8 rounded-full bg-secondary border border-border flex items-center justify-center"><MessageSquare className="w-[18px] h-[18px] text-muted-foreground" strokeWidth={1.5} /></div>
                  Feedback
                </button>
                ...
                <button onClick={async (e) => { e.stopPropagation(); await signOut(); navigate("/login"); onNavigate?.(); setProfileMenuOpen(false); }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-red-50 transition text-left text-[13px]" style={{ color: '#DC2626' }}>
                  <div className="w-8 h-8 rounded-full bg-red-50 border border-red-200 flex items-center justify-center"><LogOut className="w-[18px] h-[18px] text-red-500" strokeWidth={1.5} /></div>
                  Log out
                </button>
              </motion.div>
            </>
                  ...
                  {moreOpen && (
                    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} transition={{ duration: 0.15 }} className="absolute bottom-full right-0 mb-2 mr-1 w-48 rounded-2xl border border-border bg-card p-1.5 shadow-xl">
                      {moreItems.map((item) => {
                        ...
                        return (
                          <button key={item.path} onClick={() => { setMoreOpen(false); navigate(item.path); }} className={cn("flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium transition-colors", itemActive ? "bg-accent text-accent-foreground" : "text-foreground hover:bg-secondary")}>
                            <ItemIcon className="h-5 w-5" />{item.label}
                          </button>
                        );
                      })}
                    </motion.div>
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  // FIX: Start collapsed=false and don't auto-collapse — that was causing the "dashboard pops and deletes"
  ...
  // Removed auto-collapse timer — it was causing the dashboard to "flash" (pop and delete)
